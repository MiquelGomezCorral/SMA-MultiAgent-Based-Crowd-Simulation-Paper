...
Mencionar que las salidas NO están incrustadas el las paredes, sino que son celdas libres en los bordes de la malla. Los agentes pueden salir del escenario al llegar a una celda de salida y esto se puede hacer de 5 en 5. Esto quiere decir que el flujo máximo que cada salida puede soportar es de 5 agentes por tick.
\begin{figure}[H]
	\centering
	...
	\label{fig:pos_salidas.png}
\end{figure}

Es caso de haberlas incrustado, sería de tres. Es algo a mencionar con lo que NO se experimenta en este trabajo, pero que podría ser interesante para futuros estudios. Somos generosos y cualquier agente al lado de usa salida puede evacuar, lo que puede no ser realista en escenarios de alta densidad ya que las puertas físicas tienen un ancho limitado.
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \textbf{Variaciones}: Ratio de agentes agresivos [0\%, 10\%, 25\%, 50\%, 80\%, 100\%] manteniendo el resto como Polite sin lentos.
    ...
    \begin{itemize}
        ...
        \item \textbf{Variaciones}: Ratio de agentes lentos [0\%, 10\%, 25\%, 50\%, 80\%, 100\%] manteniendo el resto como Polite sin agresivos.
        \item \textbf{Densidades Probadas}: 125, 200 y 300 agentes.
    \end{itemize}
...
El factor de bloqueo es la métrica clave para entender la congestión. Se calcula como la cantidad de ticks en los que se ha intentado mover pero no ha podido, multiplicado por un factor específico para cada tipo de agente más un componente de influencia de los vecinos.
...
Cuando la congestión se resuelve, el contador se reduce rápidamente, permitiendo que el agente vuelva a su comportamiento normal. Rangos de menos de 1 no afectan prácticamente al movimiento, mientras que valores mayores de 3 empiezan a tener un impacto significativo.
...
\begin{enumerate}
    ...
    \item \textbf{Comportamiento Agresivo}: Efecto de la competitividad de los agentes (0\% a 100\% agresivos). NO habrá agentes lentos.
    \item \textbf{Agentes Lentos}: Impacto de agentes con movilidad reducida (0\% a 100\% lentos). NO habrá agentes agresivos.
...
La tasa de evacuación aumenta de 2.40 agentes/step (educados) a 2.76 agentes/step (agresivos), un incremento del 15\%. Más sorprendente aún, el \textbf{factor de bloqueo} disminuye del 0.73 (educados) al 0.61 (agresivos), una reducción del 16\%. Se ven patrones similares independientemente de la cantidad inicial de agentes (125, 200, 300).

Ahora, si nos fijamos en el tiempo medio de evacuación por tipo de agente, los agresivos siempre evacuan más rápido que los educados y además, se ve de forma significativa que los educados tardan menos cuando hay más agresivos en la mezcla.

\begin{figure}[H]
	\centering
	\includegraphics[width=1\linewidth]{images/plot_aggressive_impact.png}
	\caption{Impacto proporción agentes agresivos en tiempo total de evacuación y promedio por tipo de agente en Seats con 200 agentes. Rojo: Agresivos, Azul: Educados, Verde: Promedio total. El ratio es de 0 a 1, siendo 1 la cantidad máxima de agresivos (mitad de la población).}
...
Este resultado desafía la intuición común de que la cooperación siempre optimiza el flujo en multitudes, demostrando que en modelos discretos el movimiento rápido supera a ser cuidadoso.
...
Pasa lo mismo que con los agresivos pero al revés: lo agentes, en general tardan más cuanto mayor es el porcentaje de lentos en la mezcla. Y además, los lentos siempre tardan más en evacuar que los educados. Se ven patrones similares independientemente de la cantidad inicial de agentes (125, 200, 300).

\begin{figure}[H]
	\centering
	\includegraphics[width=1\linewidth]{images/plot_slow_impact.png}
	\caption{Impacto proporción agentes lentos en el tiempo total de evacuación y promedio por tipo de agente en Seats con 200 agentes. Amarillo: Lentos, Morado: Polite, Verde: Promedio total. El ratio es de 0 a 1, siendo 1 la cantidad máxima de lentos (mitad de la población).}
	\label{fig:plot_slow_impact}
\end{figure}

\textbf{Interpretación}: La alta variabilidad observada sugiere que la \textit{posición} de los agentes lentos importa tanto como su \textit{cantidad}. Un agente lento bloqueando un pasillo estrecho o una salida tiene un impacto desproporcionado comparado con un agente lento en un espacio abierto. Esto resalta la importancia de considerar la distribución espacial de poblaciones vulnerables (personas mayores, niños, personas con movilidad reducida) en el diseño de planes de evacuación.
...
\textbf{Implicación práctica}: A* es una búsqueda informada, representa a agentes que sobrevalorar ciertas rutas basándose en la su conocimiento, mientras que BFS explora todas las opciones de manera uniforme, lo que resulta en mejores decisiones en entornos como seguir siempre las señales de evacuación. 

Ya que depende del escenario, es algo a valorar si poner señalización o no en un entorno real.
...
\begin{itemize}
    \item \textbf{Mall}: +120\% tiempo de evacuación al preferir salidas (110 vs 250 steps)
...
\textbf{Observación}: Aumentar el número de salidas mejora la eficiencia hasta saturar el flujo y llegar a un máximo donde añadir más salidas no aporta beneficios significativos.
...
\begin{figure}[H]
	...
	\caption{Impacto número de salidas en el tiempo total de evacuación en escenarios con 200 agentes. No todos los escenarios permiten el mismo número máximo de salidas.}
	\label{fig:plot_n_exits}
\end{figure}


\textbf{Interpretación}: Una vez que nl escenario tiene salidas cerca de cada borde, el cuello de botella se desplaza de las \textit{puertas} a la \textit{habitación misma}. Añadir más salidas no acelera la evacuación si los agentes no pueden llegar a ellas más rápido.

El punto óptimo parece estar alrededor de 4 salidas distribuidas uniformemente (una en cada lado). Este resultado se observa en todos los escenarios, aunque la saturación ocurre a diferentes niveles según la topología como en \textbf{Seats}, donde don 2 ya se satura.

\subsection{Ancho de Salida}

\textbf{Observación}: Ampliar el ancho de los caminos hacia las salidas reduce drásticamente los cuellos de botella, pero el impacto es \textit{dependiente del contexto}.

\textbf{Resultados} (escenario Seats, 125 agentes):

\begin{itemize}
    \item \textbf{Ancho 1 celda}: 142.3  steps, factor de bloqueo = 0.42, densidad local = 2.03
    \item \textbf{Ancho 2 celdas}: 103.0 steps (+1.6\%), factor de bloqueo = 0.35, densidad local = 1.85
    \item \textbf{Ancho 3 celdas}: 101.2 steps (+11.4\%), factor de bloqueo = 0.15, densidad local = 1.42
    \item \textbf{Ancho 4 celdas}: 99.8 steps (+0.1\%), factor de bloqueo = 0.03, densidad local = 1.06
\end{itemize}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.75\linewidth]{images/plot_width_impact.png}
	\caption{Impacto del ancho de los pasillos hacia la salida en el tiempo total de evacuación en Seats con 125 agentes.}
	\label{fig:plot_width_impact}
\end{figure}

\textbf{Resultado sorprendente}: Ampliar el ancho de salida, solo se ve una reducción significativa en el tiempo de evacuación al pasar de 1 a 2 en un 29.8\% (142.3 a 99.8 steps), mucho menos de lo esperado. A partir de ahí, las ganancias son marginales (menos del 2\% adicional), saturándose.

\textbf{Interpretación}: En entornos con obstáculos internos densos (filas de asientos), el tiempo de evacuación está dominado por la \textit{navegación interna}. Como vemos, se produce un cambio grande al pasar de pasillos estrechos a anchos (1x a 2x), pero luego, los agentes siguen mantienen el flujo, ampliar más no aporta beneficios significativos.

\textbf{Implicación de diseño}: A la hora de diseñar rutas de evacuación, es crucial identificar hasta donsde un cambio agiliza la evacuación, y no limitarse a ampliar sin más. Si se pueden añadir más butacas sin afectar la evacuación, es preferible hacerlo para maximizar la capacidad del espacio. Después, habrá que analizar que otros factores (num. agentes, comportamiento, etc.) están limitando el flujo.

\section{Análisis de Evolución Temporal}

Mientras que los resultados agregados revelan patrones a nivel de sistema, el análisis temporal paso a paso expone la \textit{dinámica} de los procesos de evacuación. 

Esta sección examina cómo evolucionan los bloqueos y la velocidad a lo largo del tiempo, enfocándose en los experimentos de \textbf{Preferencias de Salida} y \textbf{Ancho de Salida}. Las razones por la que se estudian solo estos dos casos son: son suficientes para ilustrar el análisis temporal, y el scope del trabajo no permite extenderse más.
...
El tiempo normalizado se dividió en 10 intervalos (bins) de igual amplitud: [0-10\%, 10-20\%, ..., 90-100\%], permitiendo agregar métricas en fases equivalentes del proceso de evacuación. Es como medir 'fase de inicio', 'fase de congestión', 'fase de resolución', etc.
...
\textbf{Fase 3: Resolución Prolongada (50-100\%)}: La congestión persiste durante el 32.8\% de la duración total de las simulaciones con preferencias, comparado con solo 16.2\% sin preferencias. Esto indica un \textbf{tiempo de resolución de conflictos 102\% más largo}.
...
\textbf{Degradación Temprana}: Con preferencias, la velocidad cae de 0.37 (inicio) a 0.21 (pico de congestión, 40-50\%). Sin preferencias, la caída es más gradual: de 0.37 a 0.30.

\textbf{Patrón de Recuperación}: Sin preferencias, la velocidad se recupera rápidamente después del 60\%, alcanzando 0.35 al final. Con preferencias, la recuperación es incompleta: apenas alcanza 0.28 al final. Ambas empiezan en 0.37 pero solo la sin preferencias recupera el valor inicial.

\textbf{Punto Clave}: La velocidad promedio \textit{desciende 10-20\% del tiempo normalizado \textbf{antes} de que el factor de bloqueo alcance su pico}. Esto convierte a la velocidad en una señal de alerta temprana viable para sistemas de monitoreo en tiempo real.
...
\begin{itemize}
    ...
    \item \textbf{Cascadas de Interferencia}: Un bloqueo en una ruta fuerza a agentes a moverse hacía más direcciones cuando están en modo pánico, cruzando más rutas y bloqueando aún más.
    \item \textbf{Saturación Prematura}: El espacio libre por el que circulan los agentes se reduce al convertirse en una 'intersección de alto tráfico'.
\end{itemize}
...
En el escenario \textbf{Seats}, ampliar los pasillos hacia las salidas de 1 una celda a 4, produce cambios mínimos en el tiempo total de evacuación (<1\% de diferencia), pero el análisis temporal revela \textit{cambios significativos en la distribución de la congestión}:

\textbf{Ancho 1 celda}: Factor de bloqueo pico = 1.24 en el rango 70-80\%

\textbf{Ancho 2 celdas}: Factor de bloqueo pico = 1.18 en el rango 60-70\%

\textbf{Ancho 3 celdas}: Factor de bloqueo pico = 1.14 en el rango 50-60\%

\textbf{Ancho 4 celdas}: Factor de bloqueo pico = 1.10 en el rango 40-50\%
...
\begin{itemize}
    \item \textbf{Ancho 1 celda}: velocidad promedio = 0.28
    \item \textbf{Ancho 4 celdas}: velocidad promedio = 0.28
\end{itemize}
...
Es por esto, que vemos la utilidad de analizar la evolución temporal, ya que nos permite ver estos detalles que no se aprecian en los resultados agregados. Se demuestra así el potencial que tienen este tipo de herramientas para el análisis de evacuaciones.
...
- Se ha demostrado que se puede hacer uso del sistema para modelar evacuaciones en diferentes escenarios y con diferentes tipos de agentes, analizar el efecto de la geometría del entorno y que se pueden experimentar con parámetros específicos de los escenarios
- Que se puede hacer un análisis detallado de la evolución temporal de las métricas clave durante la simulación, revelando dinámicas de congestión y patrones de flujo que no son evidentes en los resultados agregados.
...
No se han observado colas en las salidas en los escenarios estudiados como sí pasa en la vida real. Esto puede ser debido a la generosidad que hemos dado a que cualquier agente en alguna de las 5 celdas alrededor de la salida pueda salir, lo que reduce la congestión en la puerta.
...
\begin{itemize}
    ...
    \item Investigar influencia de las posiciones de las salidas y cuantos agentes pueden salir por step en función del ancho de estas. 
\end{itemize}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                             %
%                                BIBLIOGRAFIA                                 %
